...
export class App implements AfterViewInit, OnDestroy {
	...
	async copyJson(): Promise<void> {
		try {
			if (!this.currentEdit) throw new Error("Editor not ready yet.");
			await navigator.clipboard.writeText(JSON.stringify(this.currentEdit.getEdit(), null, 2));
			this.toast("JSON copied", "success");
		} catch (error) {
			this.toast(error instanceof Error ? error.message : "Could not copy JSON.", "error");
		}
	}
	...
	private toast(message: string, kind: "success" | "error"): void {
		...
		this.toastTimer = window.setTimeout(() => el.remove(), 3000);
	}
}
